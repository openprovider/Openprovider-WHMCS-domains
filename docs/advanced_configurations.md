...
| renewTldsUponTransferCompletion | string: ""(empty string)                   | Enter TLDs split by a comma ("nl,eu,be") The module will alway try to renew TLDs in this list as soon as transfer is completed. This is useful for TLDs which don't include an automatic renewal with domain transfer. Note that this will incur a cost in your Openprovider account |
...
|Parameter Name|Value type and default|Purpose|
|:-----|-----|     |
|syncAutoRenewSetting| boolean: true | Whatever autorenew status your client has set for the domain in WHMCS will be passed to Openprovider. if the domain has "autorenew" status in WHMCS the domain will be set as "default" autorenew in Openprovider. |

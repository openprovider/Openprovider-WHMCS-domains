...
| OpenproviderPremium             | boolean: **read-only**                            | **Do not edit here.** This value is synchronized from the WHMCS Admin **Premium Domains** toggle (System Settings → Domain Pricing). Use the Admin UI to enable/disable premium domain sales. Ensure the same currency in both WHMCS and Openprovider for correct pricing. |
